...
namespace Accounts
{
    public partial class FrmUserMaster : Form, Entry 
    {
        ...
        void CreateUser_Master()
        {
            try
            {
                if (MyBase.Check_Table("Projects_User_Master") == false)
                {
                    MyBase.Run("Create table Projects_User_Master (ID numeric(8), User_Code numeric(8), User_Name Varchar(200), User_Address1 varchar(150), User_Status varchar(5), new_EmpCode numeric(4), New_Syscode numeric(4), New_DateTime Date,alter_EmpCode numeric(4), alter_Syscode numeric(4), alter_DateTime Date, Company_Code numeric(2), Year_Code Varchar(10))");
                }
            }
            catch (Exception ex)
            {
        ...
        void Load_User_Level_Socks()
        {
            ...
            try
            {
                MyBase.Load_Data("Select * from Projects.dbo.User_Level_PRojects Order by User_Level_Code", ref Dt);
            ...
            try
            {
                ...
                else if (e.KeyCode == Keys.Down)
                {
                    if (this.ActiveControl.Name == "TxtCustomerName")
                    {
                        ...
                        Str = "select A.TNo, A.Name, A.DesignationName, A.DeptName , A.Emplno, B.Emplno From Vaahini_Erp_Gainup.Dbo.MIS_Employee_Basic() A Left Join PRojects.dbo.Projects_User_Master B on B.Emplno=A.Emplno Where  CatCode in(5,6) and Tno not like '%Z'";
                    ...
                    {
                        Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select Location", "Select Locaton_Name Location, Location_Code Code from Projects.dbo.User_Location_master order by Locaton_Name", string.Empty, 200, 90);
                        if (Dr != null)
                        {
        ...
        public void Entry_New()
        {
            try
            {
                ...
                {
                    Code = Convert.ToString(MyBase.MaxWOCC("projects_User_Master", "User_Code", "")); 
        ...
        public void Entry_Save()
        {
            ...
            try
            {
                if (TxtCustomerName.Text.Trim() != String.Empty && TxtUserPass.Text.Trim() != String.Empty)
                {
                    ...
                    if (TxtUserPass.Text.Length < 2)
                    {
                        MessageBox.Show("Minimum 2 Characters ...!");
                    ...
                    if (MyBase.Get_RecordCount("PRojects.dbo.PRojects_User_Master", "USer_Name = '" + TxtCustomerName.Text + "' and USer_Code <> " + Code) > 0)
                    {
                    ...
                    {
                        Code = MyBase.MaxWOCC("PRojects.dbo.PRojects_User_Master", "User_Code", "").ToString();
                    }
                    else
                    {
                        MyParent.New_UserCode = MyBase.GetEntryDetails_User("PRojects.dbo.PRojects_User_Master", "user_Code = " + Code);
                        MyParent.New_Today = MyBase.GetEntryDetails_Date("PRojects.dbo.PRojects_User_Master", "user_Code = " + Code);
                        MyParent.New_SysCode = MyBase.GetEntryDetails_Sys("PRojects.dbo.PRojects_User_Master", "user_Code = " + Code);
                    }
                    Sql = "Delete From PRojects.dbo.PRojects_User_Master where User_Code = " + Code;
                    if (MyParent._New == true)
                    {
                        Sql1 = "Insert into PRojects.dbo.PRojects_User_Master values (" + Code + "," + Code + ",'" + TxtCustomerName.Text + "','" + MyBase.Ascii(TxtUserPass.Text) + "','" + Block + "'," + MyParent.UserCode + "," + MyParent.SysCode + "," + MyParent.Today + ", " + MyParent.UserCode + "," + MyParent.SysCode + "," + MyParent.Today + "," + MyParent.CompCode + ",'" + MyParent.YearCode + "', " + TxtLocation.Tag.ToString() + ", " + CmbUserLevel.SelectedValue.ToString() + ", " + TxtName.Tag.ToString() + ",'" + TxtMailID.Text + "')";
                    }
                    else
                    {
        ...
        void Fill_Datas(DataRow Dr)
        {
            try
            {
                if (Dr != null)
                {
                    ...
                    MyParent.Vew_Help("PRojects.dbo.PRojects_User_Master", "USer_Code = " + Code);
                }
                else
                {
                    Code = String.Empty; 
