...
namespace SelectionTool
{
    public partial class FrmSelectionTool : Form
    {
        ...
        public DataTable Selected_Data;
        ...
        public void Grid_Detail_Multiple(bool ViewMode, String Sql, params int[] Col)
        {
            try
            {
                Multiple = true;

                Grid_Data_Multiple(Sql);
                Column_Width_Multiple (dataGridView1.Columns.Count, Col);
                Fill_Columns();
                ItemCount();
        ...
        public void Grid_Data_Multiple(String Sql)
        {
            try
            {
                //===============================================================
                ...
                dataGridView1.Columns.Insert(1, Check);
                dataGridView1.Columns["Status"].DefaultCellStyle.BackColor = System.Drawing.Color.Yellow;
        ...
        void Column_Width_Multiple(int Column, params int[] nos)
        {
            try
            {
        ...
        void Fill_Data_to_SelectData()
        {
            try
            {
                Selected_Data = Dv.Table.Clone();

                for (int i = 0; i <= Dv.Table.Rows.Count - 1; i++)
                {
                    if (dataGridView1.Rows[i].Cells[0].Value == "true")
                    {
                        Selected_Data.ImportRow(Dv.Table.Rows[i]);
                    }
                }

                Selected_Data.Columns.RemoveAt(0);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (Multiple)
                {
                    if (dataGridView1.Rows[dataGridView1.CurrentCell.RowIndex].Cells[0].Value == "true")
                    {
