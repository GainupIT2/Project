...
namespace Accounts
{
    ...
    {       
            ...
            try
            {
                ...
                {                   
                    if (this.ActiveControl.Name == "TxtBuyer")
                    {
                       ...
                       {
                           Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select Buyer", "Select Distinct Party Buyer, PArty_Code BuyerID From Projects.Dbo.Project_ORder_Fn() ORder by 1", String.Empty, 350);                       
                       }                       
                       else
                       {
                           Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select Buyer", "Select Distinct Party Buyer, PArty_Code BuyerID From Projects.Dbo.Project_ORder_Fn() ORder by 1", String.Empty, 350);                       
        ...
        {            
            try
            {
                Mybase.Load_Data("SELECT 0 AS SLNO,  O_Slno,  Access_Type ACCESS, Quantity UNIT,  Cast(0 as Numeric (25, 2)) PREVIOUS, Cast(0 as Numeric (25, 2)) as NOW_, Cast(0 as Numeric (25, 2)) AMOUNT FROM PROJECTS.DBo.Budget_Approval_Summary_Project() WHERE ORDER_NO in (" + OCN_List + ")  Group By O_Slno , Access_Type , Quantity , App_Quantity Order By Access_Type, O_Slno ", ref Dt_Sum);
        ...
        void Fill_Approved_Amount()
        {
            try
            {
                for (int i = 0; i <= Dt_Sum.Rows.Count - 1; i++)
                {
                    ...
                    for (int j = 0; j <= Dt_Budget.Rows.Count - 1; j++)
                    {
                        if (Dt_Sum.Rows[i]["O_Slno"].ToString() == Dt_Budget.Rows[j]["ItemID"].ToString())
                        {
                        ...
                        {                            
                                if (Grid_Budget_PR[k, i].Style.BackColor == Color.Green)
                                {
                                    if (Convert.ToDouble(Grid_Budget_PR[k, i].Value.ToString()) != 0)
                                    {
                                        ...
                                        //else
                                        //{
                                        //    Queries[Array_Index++] = "Update Projects.Dbo.Project_Planning_Material_Details Set Approval_Flag = 'T' , Approval_Time = Getdate(), Approval_System = Host_Name(), App_Pur_Rate_Conv = " + Convert.ToDouble(Grid_Budget_PR[k, i].Value.ToString()) + " Where RoWID In (" + Convert.ToDouble(Dt_Budget_ID.Rows[i][k].ToString()) + ") ";                                        
                                        //}
