...
namespace Accounts
{
    public partial class FrmVehicleCarBooking : Form,Entry
    {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (MyParent._New == true)
                    {
                        ...
                        {
                            CarBookingSMS(Convert.ToInt32(TxtBNo.Text));
                        }
                    }
                    else
                    {
        ...
        // Vehicle Booking SMS to the Client and Admin
        void CarBookingSMS(Int32 bookid)
        {
            try
            {
                String Emplno = String.Empty;
                ...
                Int32 bid = Convert.ToInt32(bookid);
                String Bdate = String.Empty;
                String Message = String.Empty;
                //String Company = String.Empty;
                //String eldays = String.Empty;
                //String netamt = String.Empty;
                //String Year = String.Empty;
                //String Bonus = String.Empty;
                DataTable Dt = new DataTable();

                //String Str = "Select Name,Tno,BookNo,Bookdate,Fromadd,Toadd,PhoneNumber,rowid from Vaahini_erp_gainup.dbo.vehicle_Booking_sms_fn() where RoWid=" + bookid + "";
                String Str = "Select Name,BookNo,convert(varchar(15),Bookdate,103)Bookdate,Fromadd,Toadd,Persons,PhoneNumber from VAAHINI_ERP_GAINUP.dbo.Car_Vehicle_Booking_req_SMS() where Bookno=" + bid + "";
                //String Str = "Select Name,BookNo,convert(varchar(15),Bookdate,103)Bookdate,Fromadd,Toadd,Persons,PhoneNumber from VAAHINI_ERP_GAINUP.dbo.Car_Vehicle_Booking_req_SMS("+bid+")";
                //if (TxtCompany.Text != String.Empty)
                //{
                //    Str = Str + " And CompCode=" + TxtCompany.Tag + "";
                //}
                //if (TxtEmplno.Text != String.Empty)
                //{
                //    Str = Str + " And Emplno=" + TxtEmplno.Tag + "";
                //}
                MyBase.Load_Data(Str, ref Dt);


                String[] Quries = new String[Dt.Rows.Count * 10];
                if (Dt.Rows.Count > 0)
                {
                    //ProgressBarInitialize(Dt);
                    //StatusLbl.Text = Dt.Rows.Count.ToString();
                    for (int i = 0; i <= Dt.Rows.Count - 1; i++)
                    {
                        //Emplno = Dt.Rows[i]["tno"].ToString();
                        EmpName = Dt.Rows[i]["name"].ToString();
                        //Designation = Dt.Rows[i]["DESIGNATION"].ToString();
                        PhoneNumber = Dt.Rows[i]["PhoneNumber"].ToString();
                        From = Dt.Rows[i]["Fromadd"].ToString();
                        To = Dt.Rows[i]["Toadd"].ToString();
                        Bdate = Dt.Rows[i]["Bookdate"].ToString();
                        //Message = "Car Booking Request " + "%0A" + "BookId:" + bookid + "%0A" + "BookDate:" + Bdate + "%0A" + "TNO:" + Emplno + "%0A" + "Name:" + EmpName + "%0A" + "From:" + From + "%0A" + "To:" + To + "";
                        Message = "Car Booking Req:" + "%0A" + "Name:" + EmpName + "%0A" + "Phone:" + PhoneNumber + "%0A" + "BookNo:" + bid + "%0A" + "Date:" + Bdate + "%0A" + "From:" + From + "%0A" + "To:" + To + "";
                        if (Message.Length > 160)
                        {
                            Message = Message.Substring(0, 159);
                        }

                        String SmsStatus = "NOTSEND";
                        //Ramalingam Sir PhoneNumber
                        //9543930051
                        ...
                        //String Str3 = "Insert Into VAAHINI_GAINUP_COMPLIANCE.dbo.BonusSmsLog(Emplno,Message,SmsStsuts,SmsSenderUsercode,Module,MobileNumber,Year_Code) Values(" + Emplno + ",'" + Message + "','" + SmsStatus + "'," + MyParent.UserCode + ",'" + ModuleName + "','" + PhoneNumber + "'," + Year + ")";
        ...
        // Check internet connection - Code Taken from Payroll Module
        // For the purpose of  Sending SMS to the Client and Admin for the Vehicle Booking
        // Enabled By M.Balasubramani
            ...
            try
            {
                ...
                if (Connection == true)
                {
                    ...
                    if (DtChk.Rows.Count > 0)
                    {
                        ...
                        //Wc.OpenRead("http://www.google.com");
        ...
        //SendSMS Function Copied from VPayroll
        public Boolean SendSMS(String Mobile, String SMS_Text)
        {
            try
            {
                ...
                if (Sms_Dt.Rows.Count > 0)
                {
                    ...
                    try
                    {
                        //sr = new StreamReader(objRequest.GetRequestStream(), Encoding.UTF8, true, 1024, true);
                        //myWriter = new StreamWriter(objRequest.GetRequestStream().ToString(),true, Encoding.UTF8, 1024);
        ...
        public void Entry_Print()
        {
            try
            {
                ...
                Str = "Select EntryNo , BookNo , Status , NAME , TNO,  TYPE , BOOKDATE , FROMADD , TOADD , PERSONS , REMARKS , VEHNO , VEHTYPE , DRIVERNAME , PHONENO , CompName , PDate , Approve,Cancel_Booking,Division , PersonName , TnoDt , Company , EntryDate , Designation , convert( varchar(10), GETDAte(), 101) + stuff( right( convert( varchar(26), GETDAte(), 109 ), 15 ), 7, 7, ' ') PrintDate From VAAHINI_ERP_GAINUP.dbo.VehicleCar_Booking_Details_ForMail() Where BookNO=" + TxtBNo.Text + "";
                ...
                MyBase.Execute_Qry(Str, "Vehicle_Booking_Car_Rpt");
                CrystalDecisions.CrystalReports.Engine.ReportDocument ORpt = new CrystalDecisions.CrystalReports.Engine.ReportDocument();
                ORpt.Load(System.Windows.Forms.Application.StartupPath + "\\Vehicle_Booking_Car_Rpt.rpt");
                ...
                MyParent.FormulaFill(ref ORpt, "Heading", "Vehicle Car Booking Report");
                ...
                {
                    MyParent.CReport(ref ORpt, "Vehicle Car Booking Report...!");

                }
                else if (Res == DialogResult.No)
                {
                    TempDt1 = new DataTable();
                    Str = "Select EntryNo , BookNo , Status , NAME , TNO,  TYPE , BOOKDATE , FROMADD , TOADD , PERSONS , REMARKS , VEHNO , VEHTYPE , DRIVERNAME , PHONENO , CompName , PDate , Approve,Cancel_Booking,Division , PersonName , TnoDt , Company , EntryDate , Designation,  VAAHINI_ERP_GAINUP.dbo.Get_No_Of_Vehicle_Booking_Car_ForGivenDate('" + String.Format("{0:dd-MMM-yyyy}", DtpDate1.Value) + "') BookNos From VAAHINI_ERP_GAINUP.dbo.VehicleCar_Booking_Details_ForMail() A Where BookNO=" + TxtBNo.Text + "";
                    MyBase.Load_Data(Str, ref TempDt1);
                    if (TempDt1.Rows.Count == 0)
                    {
                        MessageBox.Show("No Record Found..!");
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            //Body.Append("<br/>NUMBERS FOR GOODS BOOKING ON THIS DATE :" + TempDt1.Rows[0]["Pdate"]);
                            //Body.Append("<br/>BOOK NUMBERS :" + TempDt1.Rows[0]["BookNos"].ToString());
                            MyParent.CReport_Normal_PDF(ref ORpt, "Vehicle Car Booking..!", "C:\\Vaahrep\\Vehicle_Booking_Car_Rpt.Pdf", false);
                            MyBase.sendEMailThroughOUTLOOK("Safety@gainup.in", "maingate@gainup.in", "Vehicle Car Booking On :" + String.Format("{0:dd/MMM/yyyy}",TempDt1.Rows[0]["Pdate"]), Body.ToString(), "C:\\Vaahrep\\Vehicle_Booking_Car_Rpt.Pdf");
                            return;
        ...
        public void Fill_Datas()
        {
            try
            {
                ...
                if (TxtType.Text.ToString().Trim().Contains("BUYER"))
                {
                    ...
                    String St1 = "Select A.NAme,A.Rowid From VAAHINI_ERP_GAINUP.dbo.Buyer_For_CarBooking(" + MyParent.CompCode + ",'" + String.Format("{0:dd-MMM-yyyy}", DtpDate1.Value) + "')  A Left join VAAHINI_ERP_GAINUP.dbo.BookingMas B On A.Rowid=B.Buyerid Where B.Rowid=" + Code + "";
                    MyBase.Load_Data(St1, ref Dtg);
                    ...
                    TxtBuyerName.Text = Dtg.Rows[0]["NAme"].ToString();
                    TxtBuyerName.Tag = Dtg.Rows[0]["Rowid"].ToString();
                }
                else
                {
                    TxtNoofPersons.Enabled = false;
                ...
                else if (e.KeyCode == Keys.Down)
                {
                    ...
                    else if (this.ActiveControl.Name == "TxtBuyerName")
                    {
                        if (MyParent._New == true || MyParent.Edit == true)
                        {
                            ...
                            Str = "Select Name Type,Rowid From VAAHINI_ERP_GAINUP.dbo.Buyer_For_CarBooking("+MyParent.CompCode+",'"+String.Format("{0:dd-MMM-yyyy}",DtpDate1.Value)+"')";
                            Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select Type", Str, String.Empty, 200);
                            if (Dr != null)
                            {
